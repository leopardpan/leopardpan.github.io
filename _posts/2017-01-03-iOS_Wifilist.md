...
　　知道了原理有啥用呢，能实现么？说出来和做出来是两个维度的事情。好吧现在就遇到问题了，移动设备如今主要是 Andorid 和 iOS, Android 上可以直接扫描 Wifi 列表获取相关信息，网上找找, 所以说会原理不一定会技术实现，我也就只能讲讲 iOS 的技术实现了。                
...
>* 2、申请包含 Network Extension 的描述文件（证书）
>* 3、Xcode Info.plist 里 Required background modes 添加 一个 network-authentication(item)
>* 4、Demo.entitlements（Demo是项目名称） 里添加 Key-Value: com.apple.developer.networking.HotspotHelper -> YES
>* 5、代码 [NEHotspotHelper registerWithOptions:nil queue:queue handler: ^(NEHotspotHelperCommand * cmd) {} ，  回调里的 cmd.networkList 就是 Wifi 列表（每次系统 Wifi 列表有变化的时候就会回调。）。
